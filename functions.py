...
def centroid(df,method = 'all'): # method is 'all' or int (number of random picks)
    mask = np.where(np.sum(df, axis = 0) > 0)[0]
...
def opti(df,method = 'all'): # method is 'all' or int (number of random picks)
    ...
    while id <= N :
        ...
        spec_ord = np.argsort(np.sum(df.T/(id + 1 + norm**2), axis = 1))
        f1 = 0
        spec_id = spec_ord[-id-1:]
        assemblage[spec_id] = 1
    ...
    for cl in tqdm(range(N_cluster)):
        if sum((clusters == cl)) != 0:
            if method == 'medoid' :
                ...
                spec_k, f1 = mc_uniform(cluster_dt[(clusters == cl)],nb = 1000, method = 1000)
            elif method == 'mc_distri':
                spec_k, f1 = mc_distri(cluster_dt[(clusters == cl)], nb = 1000, method = 1000)
            elif method == 'mc_norm':
                spec_k, f1 = mc_norm(cluster_dt[(clusters == cl)], nb = 1000, method = 1000)
            elif method == 'centroid' :
                spec_k, f1 = centroid(cluster_dt[(clusters == cl)], method = 1000)
            elif method == 'barycenter' :
                spec_k, f1 = euclidian_bar(cluster_dt[(clusters == cl)], method = 1000)
            elif method == 'opti' :
                spec_k, f1 = opti(cluster_dt[(clusters == cl)], method = 1000)
            else : 
                raise NameError
            Score[cl] = f1
            Ck[cl] = spec_k
